...
# 读取JSON文件
json_path = '/workspace/vllm/benchmarks/result_reproduct12.9-3.10/baseline/second_round/collected_metrics.json'
with open(json_path, 'r') as f:
    data = json.load(f)
# 从JSON数据中提取p95_ttft值
baseline_p95_ttft = data['p95_ttfts']

json_path = '/workspace/vllm/benchmarks/result_reproduct12.9-3.10/envelope/second_round/collected_metrics.json'
with open(json_path, 'r') as f:
    data = json.load(f)
# 从JSON数据中提取p95_ttft值
envelope_p95_ttft = data['p95_ttfts']

rate = np.arange(1, 16, 1)

plt.figure(figsize=(10, 6))  # 设置图的大小
plt.plot(rate, baseline_p95_ttft, marker='o', label='Baseline', color='red')  # 绘制线图，并添加数据点标记
plt.plot(rate, envelope_p95_ttft, marker='s', label='Envelope', color='blue')  # 绘制线图，并添加数据点标记
plt.xlabel('Rate')  # x轴标签
plt.ylabel('p95 TTFT')  # y轴标签
plt.title('Rate vs p95TTFT')  # 图表标题
plt.grid(True)  # 添加网格
plt.legend()  # 确保调用了这行代码
plt.savefig('benchmarks/rate_vs_p95_ttft.png')
